-- Accept empty date strings from mixed client payloads during sync.
...
begin
  ...
  if v_reference_type = 'JournalEntry' then
    ...
    insert into public.journal_entries (
      ...
      coalesce(nullif(v_data ->> 'date', '')::date, current_date),
      nullif(v_data ->> 'referenceNumber', ''),
      nullif(
        coalesce(v_data ->> 'referenceDate', v_data ->> 'reference_date'),
        ''
      )::date,
